...
from .form import StateForm, CategoryForm, FeaturesForm, PaymentTypeForm, PaymentForm
...
def payment_details(request):
    category= Category.objects.filter(blocked=False)
    payments = Payment.objects.filter(blocked=False)
    context = {
        'payments': payments,
            "category":category}
    return render(request, 'other/details_payment.html', context)
...
def create_payment(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('details_payment')
    else:
        form = PaymentForm()
    return render(request, 'other/create_payment.html', {'form': form})

@login_required(login_url="login-page")
def update_payment(request,pk):
    payment=Payment.objects.get(payment_no=pk)
    if request.method == 'POST':
    ...
    return render(request, 'other/update_payment.html', {'form': form})
