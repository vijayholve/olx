...
@login_required(login_url="login-page")
def o_details(request):
    category= Category.objects.filter(blocked=False)
    os = O.objects.filter(blocked=False)
    context = {
        'os': os,
            "category":category}
    return render(request, 'other/details_o.html', context)

@login_required(login_url="login-page") 
def create_o(request):
    if request.method == 'POST':
        form = OForm(request.POST)
        if form.is_valid():
            form.save()
            return redirect('details_o')
    else:
        form = OForm()
    return render(request, 'other/create_o.html', {'form': form})

@login_required(login_url="login-page")
def update_o(request,pk):
    o=O.objects.get(id=pk)
    if request.method == 'POST':
        form = OForm(request.POST, instance=o)
        if form.is_valid():
            form.save()
            return redirect('details_o')
    else:
        form = OForm( instance=o)
    return render(request, 'other/update_o.html', {'form': form})

@login_required(login_url="login-page")
def delete_o(request,pk):
    o=O.objects.get(id=pk)
    o.block()
    return redirect('details_o')