...
from product.models import Product,State,Category
...
@login_required(login_url="login-page")
def update_state(request,pk):
    state=Category.objects.get(id=pk)
    if request.method == 'POST':
        category = CategoryForm(request.POST, instance=state)
        if form.is_valid():
            form.save()
            return redirect('details_state')
    else:
        form = CategoryForm( instance=Category)
    return render(request, 'other/update_state.html', {'category': category})
