...
from .form import StateForm, CategoryForm, FeaturesForm, PaymentTypeForm, Form, OrderForm
...
def transaction_details(request):
    category= Category.objects.filter(blocked=False)
    transactions = Transaction.objects.prefetch_related('order_set').all()
    context = {
        'transactions': transactions,
            "category":category}
    return render(request, 'other/details_transaction.html', context)
...
def create_(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('details_')
    else:
        form = Form()
    return render(request, 'other/create_.html', {'form': form})
...
def update_(request,pk):
    ...
    if request.method == 'POST':
        ...
        form = Form( instance=)
    return render(request, 'other/update_.html', {'form': form})
