...
@login_required(login_url="login-page")
def pay,e_details(request):
    category= Category.objects.filter(blocked=False)
    pay,es = Pay,e.objects.filter(blocked=False)
    context = {
        'pay,es': pay,es,
            "category":category}
    return render(request, 'other/details_pay,e.html', context)

@login_required(login_url="login-page") 
def create_pay,e(request):
    if request.method == 'POST':
        form = Pay,eForm(request.POST)
        if form.is_valid():
            form.save()
            return redirect('details_pay,e')
    else:
        form = Pay,eForm()
    return render(request, 'other/create_pay,e.html', {'form': form})

@login_required(login_url="login-page")
def update_pay,e(request,pk):
    pay,e=Pay,e.objects.get(pay,e_no=pk)
    if request.method == 'POST':
        form = Pay,eForm(request.POST, instance=pay,e)
        if form.is_valid():
            form.save()
            return redirect('details_pay,e')
    else:
        form = Pay,eForm( instance=pay,e)
    return render(request, 'other/update_pay,e.html', {'form': form})

@login_required(login_url="login-page")
def delete_pay,e(request,pk):
    pay,e=Pay,e.objects.get(pay,e_no=pk)
    pay,e.block()
    return redirect('details_pay,e')