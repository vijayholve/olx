...
def pay_details(request):
    category= Category.objects.filter(blocked=False)
    pays = Pay.objects.filter(blocked=False)
    context = {
        'pays': pays,
            "category":category}
    return render(request, 'other/details_pay.html', context)
...
def create_pay(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('details_pay')
    else:
        form = PayForm()
    return render(request, 'other/create_pay.html', {'form': form})

@login_required(login_url="login-page")
def update_pay(request,pk):
    pay=Pay.objects.get(pay_no=pk)
    if request.method == 'POST':
    ...
    return render(request, 'other/update_pay.html', {'form': form})

@login_required(login_url="login-page")
def delete_pay(request,pk):
    pay=Pay.objects.get(pay_no=pk)
    pay.block()
    return redirect('details_pay')