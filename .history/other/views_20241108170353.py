...
@login_required(login_url="login-page")
def features_details(request):
    category= Category.objects.filter(blocked=False)
    featuress = Features.objects.filter(blocked=False)
    context = {
        'featuress': featuress,
            "category":category}
    return render(request, 'other/details_features.html', context)
...
def create_features(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('details_features')
    else:
        form = FeaturesForm()
    return render(request, 'other/create_features.html', {'form': form})
...
def update_features(request,pk):
    ...
    return render(request, 'other/update_features.html', {'form': form})
