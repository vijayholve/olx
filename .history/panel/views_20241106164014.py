...
def product_detail(request):
    product = Product.objects.filter(blocked=False)
    # Get the first image associated with the product
    first_image = product.images.first()  # This will return the first related ProductImage object
    return render(request, 'product/product_detail.html', {'product': product, 'first_image': first_image})