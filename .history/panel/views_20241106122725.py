...
def admin_panel(request):
    if request.user.is_authenticated and request.user.is_staff:
        products = Product.objects.all(blocked=False)
        images = ProductImage.objects.all(blocked=False)
        users = User.objects.all(blocked=False)
...
# Prepare the labels and data lists
    ...
    total_plan=Plan.objects.count()
    total_products=products.count()
    customers=Customer.objects.all()
    total_state=State.objects.count()
    total_sold_products=Product.objects.filter(is_active=True).count
    context = {
        'all_states': all_states,
        'all_states_product_count': all_states_product_count,
            'products': products,
