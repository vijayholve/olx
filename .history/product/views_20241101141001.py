...
def product_details(request, product_id):
    ...
    for sugge_product in suggested_products:
        images = ProductImage.objects.filter(product=sugge_product).first()
        suggest_productimage[sugge_product.id] = images.image  
    for sugge_product in suggested_products:
        image = ProductImage.objects.filter(product=sugge_product).first()
        if image:
            suggest_productimage[sugge_product.id] = image.image
    context = {
        'product': product,
