...
def home(request):
    ...
    productimage = {}  # Initialize an empty dictionary to store images for each product

    for product in products:
        image = ProductImage.objects.filter(product=product).first()
        productimage[product.id] = image  # Store each product's image in the dictionary
