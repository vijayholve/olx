...
def product_details(request, product_id):
    ...
    product = get_object_or_404(Product, id=product_id)        suggested_products = Product.objects.all().exclude(id=product.id)[:9]  # Fetch up to 9 suggested products

    grouped_suggested_products = [
        list(filter(None, group)) for group in zip_longest(*[iter(suggested_products)]*3)
    ]
    product_images_all=[]
